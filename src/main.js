...
// One pasted string carries the shared-DB credentials and where to fetch the
// starter database, so a new PC needs no files hand-carried and no scripts run.
// Generated by tools/make_setup_code.py. Absent, the install is standalone.

function applySetupCode(code) {
  // Pasted codes arrive wrapped by whatever carried them — mail, chat, a
  // screenshot retyped. Strip every space and line break before decoding.
  const cfg = JSON.parse(Buffer.from(code.replace(/\s+/g, ''), 'base64url').toString('utf8'))
...
function fetchDownload(url, dest, gunzip, onProgress, depth = 0) {
  return new Promise((resolve, reject) => {
    ...
    // starts once seeding settles, so a silent socket means a dead app.
...
ipcMain.on('save-credentials', async (_, { username, password, setupCode }) => {
  ...
  if (setupCode) {
    let seedUrl = ''
    try {
      seedUrl = applySetupCode(setupCode)
      win.webContents.send('py-out',
        `Shared database configured — ${settings.remoteUrl.split('//').pop()}`)
    } catch (e) {
      win.webContents.send('py-out',
