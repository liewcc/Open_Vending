...
| **贩卖机门户账号和密码** | 程序以你的身份登录门户，下载补货报告。 |
| **安装码（Setup code）—— 仅当别人给了你** | 一长串文本。它让这台电脑接入共享拣货数据库，并下载初始数据。没有它就是独立安装，单机使用属于正常情况。 |
...
1. 弹出 **Welcome to Open Vending** 窗口，需要填三项：

   | 字段 | 填什么 |
   |---|---|
   | Username | 门户账号 |
   | Password | 门户密码 |
   | Setup code | 别人给了你就粘贴进去；**否则留空** —— 留空即独立安装，单机使用属于正常情况。 |

   点击 **Save & Connect**。凭据使用 Windows DPAPI 加密，保存在
   `%APPDATA%\open-vending\credentials.enc`。该文件与本机的这个 Windows 用户
   绑定，复制到别的电脑无法解密，所以每台电脑都要各自输入一次。

2. 如果填了安装码，程序会接入共享拣货数据库，并下载初始数据库（约 46 MB），
   进度显示在 Log 卡片里。只会执行一次，已有数据库绝不会被覆盖。

3. 程序随后登录门户，下载当天的补货报告。首次扫描需要几分钟，可以看底部的
   状态栏。

4. 扫描完成后主表格填充数据，项目目录下的 `db\vending.db` 即就位。
...
当初跳过了安装码，或者装完之后才拿到？欢迎窗口只在首次运行时出现，所以事后
接入需要两步：

1. 把拿到的 `turso.json` 放到**项目根目录**（和 `setup.bat` 同级）。该文件已被
   gitignore，不会离开本机。
2. 在项目目录下运行：
...
然后重启程序 —— 设置只在启动时读取。随时可以用这条命令确认当前用的是哪个库：

```bash
.\python\python.exe tools\enable_shared.py --status
```

这一步只把拣货记录和缓冲库存切到共享库，不会下载初始销售历史；那部分见
...
| 日志出现 *"Setup code not understood"* | 安装码输错或被截断了 —— 它是不换行的一整行。程序会以独立安装继续运行；重新索取安装码，并见[第 6 节](#6-事后接入共享数据库)。 |
| 日志出现 *"Starter database failed"* | 初始数据库链接不可达或已被撤销。其他功能一切正常，只是这台电脑没有销售历史。 |
